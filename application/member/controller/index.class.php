...
class index extends common{
	...
	public function user_pic(){
		...
		if(isset($_POST['dosubmit'])){
			...
			$upload = cm_base::load_sys_class('upload');
			if($upload->uploadfile('user_pic')){
				...
				$userpic = cmPHP_PATH.ltrim($userpic, SITE_PATH);
	...
	public function follow_dynamic(){
		...
		cm_base::load_sys_class('page','',0);
		$total = $member_content->join('cmcms_member_follow ON cmcms_member_follow.followid = cmcms_member_content.userid', 'RIGHT')->where("cmcms_member_follow.userid=$userid AND status=1")->total();
		$page = new page($total, 10);
		$res = $member_content->field('cmcms_member_content.checkid,cmcms_member_content.catid,cmcms_member_content.username,cmcms_member_content.title,cmcms_member_content.inputtime')->join('cmcms_member_follow ON cmcms_member_follow.followid = cmcms_member_content.userid', 'RIGHT')->where("cmcms_member_follow.userid=$userid AND status=1")->order('inputtime DESC')->limit($page->limit())->select();	
		$data = array();
		foreach($res as $val) {
	...
	public function fans(){
		...
		$page = new page($total, 9);
		$data = $member_follow->alias('f')->field('m.userid,m.username')->join('cmcms_member m ON f.userid=m.userid')->where(array('followid'=>$userid))->order('id DESC')->limit($page->limit())->select();	
		$pages = '<span class="pageinfo">共'.$total.'条记录</span>'.$page->getfull();
