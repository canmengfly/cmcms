...
class payment {
	...
	public function alipay($params){
		cm_base::load_common('plugin/alipay/pagepay.php');
		$params = array(
            'subject' => $params['desc'],
	...
	public function alipay_notify($params, $order){
		...
		cm_base::load_common('plugin/alipay/notify.php');
	...
	public function update_order($order, $transaction){
        ...
        if($result) {
			// 充值积分
			if($order['quantity']){
				$point = cm_base::load_model('point', 'member');
	...
	public function wechat($params){
		cm_base::load_common('plugin/wxpay/native.php');  //如需微信支付模块联系QQ 214243830
        $params = array(
            'body' => $params['desc'],
            'out_trade_no' => $params['order_sn'],
