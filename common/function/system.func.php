...
function get_memberavatar($user, $type=1, $default=true) {
	...
	if($type == 1){
		...
		$data = $member_detail->field('userpic')->join('cmcms_member b ON cmcms_member_detail.userid=b.userid')->where(array('username' => $user))->find();
	}	
