...
defined('cmPHP_PATH') or exit('Access Denied.'); 

define('IN_cmPHP', true);

//cmPHP框架路径
define('YP_PATH', dirname(__FILE__).DIRECTORY_SEPARATOR);
//cmPHP框架版本信息
define('cmPHP_VERSION', '2.0');
//cmPHP应用目录
define('APP_PATH', cmPHP_PATH.'application'.DIRECTORY_SEPARATOR);
...
//加载公用函数库
cm_base::load_sys_func('global');
cm_base::load_common('function/system.func.php');
cm_base::load_common('function/extention.func.php');
cm_base::load_common('data/version.php'); 
defined('cmCMS_SOFTNAME') or define('cmCMS_SOFTNAME', base64_decode('Q01DTVPlhoXlrrnnrqHnkIbns7vnu58='));
...
class cm_base {
	...
	private static function _load_class($classname, $path = '', $initialize = 1) {
		...
		if (empty($path)) $path = cmPHP_PATH.'cmphp'.DIRECTORY_SEPARATOR.'core'.DIRECTORY_SEPARATOR.'class';
	...
	public static function load_sys_func($func) {
		if (is_file(cmPHP_PATH.'cmphp'.DIRECTORY_SEPARATOR.'core'.DIRECTORY_SEPARATOR.'function'.DIRECTORY_SEPARATOR.$func.'.func.php')) {
			include cmPHP_PATH.'cmphp'.DIRECTORY_SEPARATOR.'core'.DIRECTORY_SEPARATOR.'function'.DIRECTORY_SEPARATOR.$func.'.func.php';
		}
	}
	...
	public static function load_common($path, $m = '') {
		if(!$m){
			if (is_file(cmPHP_PATH.'common'.DIRECTORY_SEPARATOR.$path)) {
				return include cmPHP_PATH.'common'.DIRECTORY_SEPARATOR.$path;
			}else{
				debug::addmsg(cmPHP_PATH.'common'.DIRECTORY_SEPARATOR.$path.L('does_not_exist'));
			}			
		}else{
