  ...
  <body>
	   <?php include template("index","header"); ?> 
		<div class="box">
		  ...
		  <!-- 焦点图 开始 -->
			<div id="jiaodian" class="focus">  
				<div id="fpic" class="fpic">
				  <?php $tag = cm_base::load_sys_class('cm_tag');if(method_exists($tag, 'lists')) {$data = $tag->lists(array('field'=>'title,thumb,url','modelid'=>'1','thumb'=>'1','limit'=>'3',));}?>
			...
			<!-- 焦点图 结束 -->
			  ...
			  <div class="left_bottom">
					...
					<div class="new_ct">
                    <ul>
					<?php $tag = cm_base::load_sys_class('cm_tag');if(method_exists($tag, 'tag')) {$data = $tag->tag(array('field'=>'id,tag,total','limit'=>'20',));}?>
					<?php if(is_array($data)) foreach($data as $v) { ?>
		  ...
		  <div class="center pad_0">
			  <div class="center_top">
				...
				<ul>
				  ...
				  <?php $tag = cm_base::load_sys_class('cm_tag');if(method_exists($tag, 'lists')) {$data = $tag->lists(array('field'=>'title,inputtime,url','modelid'=>'1','flag'=>'3','limit'=>'5',));}?>
				   <?php if(is_array($data)) foreach($data as $v) { ?>
			  ...
			  <div class="center_bottom new_list">
				...
				<ul>
				  ...
				  <?php $tag = cm_base::load_sys_class('cm_tag');if(method_exists($tag, 'lists')) {$data = $tag->lists(array('field'=>'title,url','modelid'=>'1','limit'=>'10',));}?>
				   <?php if(is_array($data)) foreach($data as $v) { ?>
		  ...
		  <div class="right">
			  <div class="right_top">
		         <h2 class="ind_bt"><a target="_blank" href="http://cms.canmeng.com/" class="gengduo">更多>></a>站点介绍</h2>
                  <a href="http://cms.canmeng.com/" target="_blank"><img src="<?php echo STATIC_URL;?>images/default.jpg"></a>	
				  <a href="http://cms.canmeng.com/" target="_blank" class="ptitle">本站为cmCMS的演示站点</a>
                  <p class="pic_p">cmCMS采用OOP（面向对象）方式自主开发的框架。框架易扩展，是一款高效开源的内容管理系统，产品基于PHP+Mysql架构...<a href="http://cms.canmeng.com/" target="_blank">[详细]</a></p>
		      </div>
			  <div class="right_bottom">
                  <p class="pic_p">cmCMS是一款轻量级开源的内容管理系统，采用OOP方式自主开发的框架（cmPHP）。系统易扩展，是一款高效开源的内容管理系统，产品基于PHP+Mysql架构，可运行在Linux、Windows、MacOSX、Solaris等各种平台上。<a href="http://blog.cmcms.com/guanyu/" target="_blank">[详细]</a></p>
				  <!-- 
				   page_content (单页面标签)：用于在首页或频道页调取单页的简介
				   标签用法：{   page_content(catid, 限制字数)  }
				  -->
		      </div>		
		  </div>		  
		</div>
		...
		<div class="box">
			<div class="left">
			  ...
			  <div class="list_news">
					...
					<?php $tag = cm_base::load_sys_class('cm_tag');if(method_exists($tag, 'lists')) {$data = $tag->lists(array('field'=>'title,url','catid'=>'1','limit'=>'11',));}?>
					<?php if(is_array($data)) foreach($data as $v) { ?>
		  ...
		  <div class="center">
			  ...
			  <div class="center_bottom">
				<ul>
					<!-- 这里只做演示，您可以根据自己的需求更改catid和其他参数 -->
					<?php $tag = cm_base::load_sys_class('cm_tag');if(method_exists($tag, 'lists')) {$data = $tag->lists(array('field'=>'title,url','catid'=>'2','limit'=>'11',));}?>
					<?php if(is_array($data)) foreach($data as $v) { ?>
					   <li>-<a href="<?php echo $v['url'];?>" title="<?php echo $v['title'];?>" target="_blank"><?php echo $v['title'];?></a></li>	
					<?php } ?>	
				</ul>
		  ...
		  <div class="right">
			  ...
			  <div class="list_news">
					<ul>
					<!-- 这里只做演示，您可以根据自己的需求更改catid和其他参数 -->
					<?php $tag = cm_base::load_sys_class('cm_tag');if(method_exists($tag, 'lists')) {$data = $tag->lists(array('field'=>'title,url','catid'=>'3','limit'=>'11',));}?>
					<?php if(is_array($data)) foreach($data as $v) { ?>
					   <li><a href="<?php echo $v['url'];?>" title="<?php echo $v['title'];?>" target="_blank"><?php echo $v['title'];?></a></li>	
					<?php } ?>						 
				</ul>            
         ...
         <div class="news_img">
		   <h2 class="ind_bt tit_img"><a  href="#" target="_blank" class="gengduo">更多>></a>图文资讯</h2>
			  <div class="w185_imgnews">
 				<ul>
				   <?php $tag = cm_base::load_sys_class('cm_tag');if(method_exists($tag, 'lists')) {$data = $tag->lists(array('field'=>'title,thumb,url','modelid'=>'1','thumb'=>'1','limit'=>'5',));}?>
		...
		<!--友情链接-->
		<div id="links">
			<p>友情链接  <a href="<?php echo U('link/index/init');?>">申请友链</a></p>
			 <ul>
			<?php $tag = cm_base::load_sys_class('cm_tag');if(method_exists($tag, 'link')) {$data = $tag->link(array('field'=>'url,logo,name','limit'=>'20',));}?>
			<?php if(is_array($data)) foreach($data as $v) { ?>
